...
const actions = { getTypePath, getImage, getSrc };
const getters = {  };

export const useImageGenStore = defineStore('imageGenerator', { state, actions } );
...
function getSrc(type){
    ...
    if(!this.countMap[type]?.length)
        this.$reset();//this.$patch({ countMap: ref(countMap) }); //, usedMap
...
function getTypePath(ctx){
    const { schema:s, fieldTypePlacement} = ctx;
    
    const schema = s || getTypeNameFromDrupalRecord(ctx);
    ...
    this.$reset();
    this.$patch({ countMap: ref(countMap) });

    return undefined;
}

function getTypeNameFromDrupalRecord(ctx){
